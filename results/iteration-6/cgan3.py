...
def generator(x, y_label, isTrain=True, reuse=False):
    ...
    with tf.variable_scope('generator', reuse=reuse):
        ...
        # ?,19,54,256
        lrelu1 = tf.nn.softmax(tf.layers.batch_normalization(deconv1, training=isTrain))
        ...
        # ?,19,36,256
        lrelu2 = tf.nn.softmax(tf.layers.batch_normalization(deconv2, training=isTrain))
...
def discriminator(x, y_fill, isTrain=True, reuse=False):
    with tf.variable_scope('discriminator', reuse=reuse):
        ...
        lrelu1 = tf.nn.softmax(conv1)
        print('CONV1')
        print(lrelu1.shape)
        # 2nd hidden layer
        conv2 = tf.layers.conv2d(lrelu1, 256, [1, 9], strides=(1, 1), padding='same', kernel_initializer=w_init, bias_initializer=b_init)
        lrelu2 = tf.nn.softmax(tf.layers.batch_normalization(conv2, training=isTrain))
        print('CONV2')
        print(lrelu2.shape)
        # output layer
        conv3 = tf.layers.conv2d(lrelu2, 1, [1, 9], strides=(1, 1), padding='valid', kernel_initializer=w_init)
...
def show_result(num_epoch, epoch_label, show = False, save = False, path = 'result.png', ):
    ...
    with open('results.txt', 'a+') as f, open('raw-data.txt', 'a+') as r:
        ...
        for idx,word in enumerate(test_ipa):
            ...
            for letter in word:
                raw_ipa_onehot = word.reshape(x_train.shape[1],x_train.shape[2])
        ...
        print('Word:',word_str,'\tGen. IPA:',ipa_str,'\tReal IPA:',generative_df.ipa[len(generative_df.index)-1])
...
train_ipa = []
fields = ['word', 'ipa']
train_df = pandas.read_csv('./data/8600-6-letter-non-name-words-dataset.txt', encoding='utf-8', usecols=fields)
generative_df = pandas.read_csv('./data/100-6-letter-non-name-words-dataset.txt', encoding='utf-8', usecols=fields)
...
for i in range(batch_size - 1):
    ...
    temp_z_ = np.eye(y_train.shape[2])[np.random.choice(y_train.shape[2],y_train.shape[1])].reshape(1,y_train.shape[1],y_train.shape[2],1)
fixed_y_ = onehot.reshape(batch_size, y_train.shape[1], y_train.shape[2], 1)

x = tf.placeholder(tf.float32, shape=(None, x_train.shape[1], x_train.shape[2], 1), name="Word_Spelling")
z = tf.placeholder(tf.float32, shape=(None, y_train.shape[1], y_train.shape[2], 1), name="Noise")
y_label = tf.placeholder(tf.float32, shape=(None, y_train.shape[1], y_train.shape[2], 1), name="Phonetic_Transcription")
y_fill = tf.placeholder(tf.float32, shape=(None, x_train.shape[1], x_train.shape[2], 1), name="Conditional_Word_Spelling")
isTrain = tf.placeholder(dtype=tf.bool, name="TrainingBoolean")
...
# run 'tensorboard --logdir=./logs/cgan_logs'
writer = tf.summary.FileWriter("./logs/cgan_logs", sess.graph)
